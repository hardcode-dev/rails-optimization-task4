...
1. Кэширование story в строке <%= render "articles/single_story", story: story %> в main_stories_feed.html.erb даст прирост в скорости рендеринга.
   ...
   - проверка:
     ...
     - результат: _Time taken for tests: 10.263 seconds_
       потом закешировал story в строке <%= render "articles/single_story", story: story %> в main_stories_feed.html.erb (<% cache story do %> <render> <% end %> )
     - результат стал: _Time taken for tests: 3.101 seconds_
