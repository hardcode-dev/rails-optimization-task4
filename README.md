...
Первым делом был склонирован проект dev.to и поднят локально.

Далее установлены системы мониторинга. Использовался только NewRelic. Scout тоже установил, но он что-то не заработал. Skylight был испробован раньше, trial у него я истратил, новую учетку создавать не стал. Также подключил rack-mini-profiler и rails-panel.

Дальше настроил Prometheus и Grafana. Но почему-то не нашел там метрик, как в видео-лекции (ruby_http_requests...).

Поднял приложение в local production.

Дальше проверял гипотезу в local production:

1. Кэширование story в строке <%= render "articles/single_story", story: story %> в main_stories_feed.html.erb даст прирост в скорости рендеринга.

   - Эта гипотеза взялась из задания. А перед этим, я полагаю, она произошла из мысли, что рендеринг паршалов, довольно дорогая операция, особенно ее использовать необдуманно. А значит, рендеринг нужно использовать экономно. Один из вариантов это сделать - закэшировать фрагменты, которые используются часто, а меняются редко.
   - проверка:

     Замерил с помощью AB главную страницу - команда `ab -c 10 -n 100 http://localhost:3000/`:
...
Если вы посчитаете, что применить кэширование здесь целесообразно, оформите обоснованный `PR` с этим предложением. Напишите в описании `PR`, какая была гипотеза, откуда она взялась, как вы проверяли гипотезу, какие результаты получили. Приложите скриншоты графиков мониторинга, если на них виден эффект оптимизации. - результат: _Time taken for tests: 10.263 seconds_
потом закешировал story в строке <%= render "articles/single*story", story: story %> в main_stories_feed.html.erb (<% cache story do %> <render> <% end %> ) - результат стал: \_Time taken for tests: 3.101 seconds*

     Вывод: кэширование дало сокращение времени ответов в 3 раза.
